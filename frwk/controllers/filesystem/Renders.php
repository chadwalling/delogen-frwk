...
class Render
{
    ...
    function View($file, $array = array())
    {
		...
		Render::setSuffix($file);
        if ($file){
			//$file = preg_replace("/\/views\//", "", $file);
	   		$fileT = BASE_PATH_VIEWS.$file;
            $file_and_path = BASE_PATH_VIEWS.$file;

            if (is_file($file_and_path)){
				//file exists get file contents check if <p: tags exist and loop on each tag and check to see if p tag value has an existing phtml file if so,
		    	//get its file contents and check to see if that file has any p tags and
